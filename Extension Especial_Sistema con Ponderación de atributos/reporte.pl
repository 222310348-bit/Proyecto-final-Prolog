...
% lista de descartados con razón detallada.
...
% por el módulo de ranking y muestra los candidatos aptos con su
% puntaje. Caso base: cuando la lista está vací­a
mostrar_aptos([]).
...
% Sub predicado donde se aplica la lógica de habilidades_faltantes/3 y
% la operación subtract/3 para explicar por qué no fueron
...
% un candidato específico en relación con una vacante dada.
imprimir_motivos(_, []).
